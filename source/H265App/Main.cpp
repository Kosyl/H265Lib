...
{
	//SeqParams()->initWithDefaults();
	//SeqParams()->setPicHeight(144);
	//SeqParams()->setPicWidth(176);
	//PicParams()->initWithDefaults();
	////binarizationTest("D:\\txt\\ec_summary.txt","D:\\txt\\bin - res.txt");
	//Picture f;
	//std::ifstream yuv("D:\\h265\\yuv\\akiyo_qcif.yuv");
	//f.loadFrameFromYuv(yuv);

	//f.initPartitions(IntraPartitioning::getStrategy());
	
	for (int i = 20; i < 30000; i += 30)
	{
		/*LOGLN(Logs::Quant, "###############################");
		Quant::getInstance()->test(i, 20, 8, 32);
		Quant::getInstance()->test2(i, 20, 8, 32);
		LOGLN(Logs::Quant, "###############################");
		Quant::getInstance()->test(-1 * i, 20, 8, 32);
		Quant::getInstance()->test2(-1 * i, 20, 8, 32);*/
	}
}

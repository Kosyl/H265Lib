...
{
	bool doSmoothing = ( itsCurrentPB->getSize( ) == 32 ) && SeqParams( )->getIntraSmoothingEnabled( );
...
Sample** IntraPred::calcPred( PBIntra* newPb )
{
	...
	itsReferenceValues[ INTRA_DIR_LEFT ] = itsCurrentPB->getSideRefs( INTRA_DIR_LEFT );
	itsReferenceValues[ INTRA_DIR_TOP ] = itsCurrentPB->getSideRefs( INTRA_DIR_TOP );
...
Sample **IntraPred::calcPredForceRefs( PBIntra *newPb, Sample *leftRefs, Sample *topRefs, const Sample corner )
{
	...
	deleteMatrix( itsReferenceValues );
	itsCornerValue = -1;
	itsCurrentPB = nullptr;
